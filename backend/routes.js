...
// });
router.post("/get-route", async (req, res) => {
  const { start, end, trafficData } = req.body;
  ...
  const graph = graphBuilder(coords, trafficData);

  const startKey = coords[0].join(",");
  const endKey = coords[coords.length - 1].join(",");

  const { dist, prev } = dijkstra(graph, startKey);
  const pathKeys = reconstructPath(prev, startKey, endKey);

  const optimizedCoords = pathKeys.map(k =>
    k.split(",").map(Number)
  );
